...
/* 2024-06-17
dp[i] = for k in (0, i]:
		 max(if num[i] > num[k] then dp[k] + 1 else dp[k])
*/
...
func findLongestChain(pairs [][]int) int {
	/*
		dp[i] = for j in 0 to i: if pairs[i][0] > pairs[j][1] then dp[j] + 1 else dp[i]
	*/
...
*/
/*	2024-06-19
dp[i][j] 是以nums[i]结尾，以j为等差的最长等差子序列的长度
dp[0][*] = 1
for k in [0,i):
	delta = nums[i] - nums[k]
	dp[0][delta] = 1
	dp[i][delta] = dp[k][delta] + 1
